...
const ExpenseTracker: React.FC<ExpenseTrackerProps> = ({ invoices, currencySymbol, onCreateExpense, onEditExpense, currentProfile, onUpdateProfile }) => {
   ...
   return (
      <div className="max-w-7xl mx-auto space-y-10 animate-in fade-in pb-12">
         ...
         <div className="flex flex-col md:flex-row justify-between items-end md:items-center gap-6 border-b border-slate-100 pb-6">
            ...
            <div className="flex flex-col md:flex-row gap-3 w-full md:w-auto">
               <button
                  ...
                  className={`w-full md:w-auto px-6 py-4 md:py-3 rounded-2xl font-bold transition-all flex items-center justify-center gap-2 border ${calculatorMode ? 'bg-white text-[#1c2938] border-slate-200 shadow-sm' : 'bg-white text-slate-500 border-transparent hover:bg-slate-50'}`}
               >
                  <Calculator className="w-5 h-5" />
                  <span>{calculatorMode ? 'Ocultar Calculadora' : 'Simular Costos'}</span>
               </button>

               <button
                  onClick={onCreateExpense}
                  className="w-full md:w-auto bg-[#1c2938] text-white px-6 py-4 md:py-3 rounded-2xl font-bold hover:bg-rose-500 transition-all flex items-center justify-center gap-2 shadow-xl hover:shadow-2xl active:scale-95 group"
               >
         ...
         <div className="grid grid-cols-2 lg:grid-cols-12 gap-3 md:gap-6 items-stretch">

            {/* 1. INCOME CARD */}
            <div className="lg:col-span-4 bg-white p-5 md:p-8 rounded-[1.5rem] md:rounded-[2.5rem] shadow-sm border border-slate-50 relative overflow-hidden group hover:shadow-md transition-all duration-500">
               <div className="absolute top-0 right-0 w-24 h-24 md:w-32 md:h-32 bg-[#27bea5] rounded-full blur-[40px] md:blur-[60px] opacity-10 group-hover:opacity-20 transition-opacity"></div>
               <div className="relative z-10 flex flex-col h-full justify-between">
                  <div className="flex items-center gap-2 md:gap-3 mb-4 md:mb-8">
                     <div className="p-2 md:p-3 bg-teal-50 rounded-xl md:rounded-2xl text-[#27bea5]">
                        <ArrowUpRight className="w-4 h-4 md:w-6 md:h-6" />
                     </div>
                     <p className="text-[10px] md:text-xs font-bold text-slate-400 uppercase tracking-widest">Ingresos</p>
                  </div>
                  <div>
                     <h2 className="text-xl md:text-4xl font-bold text-[#1c2938] tracking-tight mb-0.5 md:mb-1">{currencySymbol}{totalIncome.toLocaleString()}</h2>
                     <p className="text-[10px] md:text-sm text-slate-400 font-medium truncate">Facturas cobradas</p>
                  </div>
               </div>
            </div>
            ...
            {/* 2. EXPENSES CARD */}
            <div className="lg:col-span-4 bg-white p-5 md:p-8 rounded-[1.5rem] md:rounded-[2.5rem] shadow-sm border border-slate-50 relative overflow-hidden group hover:shadow-md transition-all duration-500">
               <div className="absolute top-0 right-0 w-24 h-24 md:w-32 md:h-32 bg-rose-500 rounded-full blur-[40px] md:blur-[60px] opacity-10 group-hover:opacity-20 transition-opacity"></div>
               <div className="relative z-10 flex flex-col h-full justify-between">
                  <div className="flex items-center gap-2 md:gap-3 mb-4 md:mb-8">
                     <div className="p-2 md:p-3 bg-rose-50 rounded-xl md:rounded-2xl text-rose-500">
                        <ArrowDownRight className="w-4 h-4 md:w-6 md:h-6" />
                     </div>
                     <p className="text-[10px] md:text-xs font-bold text-slate-400 uppercase tracking-widest">Gastos</p>
                  </div>
                  <div>
                     <h2 className="text-xl md:text-4xl font-bold text-[#1c2938] tracking-tight mb-0.5 md:mb-1 text-rose-500">
                        {currencySymbol}{totalExpenses.toLocaleString()}
                     </h2>
                     <p className="text-[10px] md:text-sm text-slate-400 font-medium truncate">Costos Operativos</p>
                  </div>
               </div>
            </div>

            {/* 3. PROFIT CARD (HERO) */}
            <div className="col-span-2 lg:col-span-4 bg-[#1c2938] p-6 md:p-8 rounded-[1.5rem] md:rounded-[2.5rem] shadow-xl relative overflow-hidden text-white group hover:scale-[1.01] transition-transform duration-500">
               <div className="absolute top-0 right-0 w-48 h-48 md:w-64 md:h-64 bg-[#27bea5] rounded-full blur-[60px] md:blur-[80px] opacity-20 -translate-y-1/2 translate-x-1/2"></div>

               <div className="relative z-10 flex flex-col h-full justify-between">
                  <div className="flex justify-between items-start mb-4 md:mb-6">
                     <div className="flex items-center gap-2 md:gap-3">
                        <div className="p-2 md:p-3 bg-white/10 rounded-xl md:rounded-2xl text-[#27bea5] backdrop-blur-sm">
                           <Wallet className="w-5 h-5 md:w-6 md:h-6" />
                        </div>
                        <p className="text-[10px] md:text-xs font-bold text-slate-300 uppercase tracking-widest">Utilidad Neta</p>
                     </div>
                     <div className="px-2 md:px-3 py-1 bg-white/10 rounded-full border border-white/10 text-[10px] md:text-xs font-bold text-[#27bea5]">
                        {totalIncome > 0 ? ((netProfit / totalIncome) * 100).toFixed(0) : 0}% Margen
                     </div>
                  </div>

                  <div>
                     <h2 className="text-3xl md:text-5xl font-bold tracking-tight mb-3 md:mb-4">{currencySymbol}{netProfit.toLocaleString()}</h2>
                     ...
                     <p className="text-[9px] md:text-xs text-slate-400 mt-2 md:mt-3 font-medium">Disponible para retiro</p>
                  </div>
               </div>
            </div>
         </div>
                     ...
                     <div className="relative z-10 flex flex-col items-center">
                        ...
                        {onUpdateProfile && (
                           <button
                              ...
                              className={`w-full py-3 rounded-xl font-bold flex items-center justify-center gap-2 transition-all ${rateSaved
         ...
         <div className="flex justify-center w-full">
            <div className="bg-white p-1.5 rounded-2xl shadow-sm border border-slate-100 flex overflow-x-auto max-w-full custom-scrollbar">
               <button
                  ...
                  className={`px-6 py-3 rounded-xl text-sm font-bold transition-all duration-300 whitespace-nowrap flex items-center gap-2 ${activeTab === 'TRANSACTIONS'
               ...
               <button
                  ...
                  className={`px-6 py-3 rounded-xl text-sm font-bold transition-all duration-300 whitespace-nowrap flex items-center gap-2 ${activeTab === 'PROVIDERS'
                     ? 'text-white bg-[#1c2938] shadow-md'
                     : 'text-slate-400 hover:text-slate-600 hover:bg-slate-50'
                     }`}
               >
