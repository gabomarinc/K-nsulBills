...
import {
  ...
  LayoutDashboard, FileBarChart, Users, Filter, Calendar, Download, Mail, Smartphone, CheckCircle2, Check, LayoutGrid,
...
const ReportsDashboard = ({ invoices, currencySymbol, apiKey, currentUser }: ReportsDashboardProps) => {
  ...
  const data = useMemo(() => {
    ...
    filteredInvoices.forEach(inv => {
      ...
      if (inv.status === 'Borrador' || inv.status === 'Rechazada') return;
    ...
    const ltvData = Array.from(clientMap.entries()).map(([name, val]) => ({
      name,
      revenue: val.revenue,
      count: val.count,
    ...
    const healthyInvoices = filteredInvoices.filter(i => {
      if (i.type !== 'Invoice' || ['Pagada', 'Rechazada', 'Borrador', 'Incobrable'].includes(i.status)) return false;
      const due = i.dueDate ? new Date(i.dueDate) : new Date(new Date(i.date).getTime() + 15 * 24 * 60 * 60 * 1000);
      return due >= now;
    });

    const delinquentInvoices = filteredInvoices.filter(i => {
      if (i.type !== 'Invoice' || ['Pagada', 'Rechazada', 'Borrador', 'Incobrable'].includes(i.status)) return false;
      const due = i.dueDate ? new Date(i.dueDate) : new Date(new Date(i.date).getTime() + 15 * 24 * 60 * 60 * 1000);
      return due < now;
    });
    ...
    const portfolioHealthData = [
      { name: 'Al Día', value: healthyInvoices.length, color: '#27bea5' },
      { name: 'Morosas', value: delinquentInvoices.length, color: '#ef4444' },
    ];
    ...
    return { monthlyData, productSalesData, funnelData, quoteFunnelData, scatterData, ltvData, portfolioHealthData, kpis: { totalRevenue, totalExpenses, netMargin, marginPercent, avgPaymentDays, conversionRate, churnRiskCount, activeClientsCount, healthyCount: healthyInvoices.length, delinquentCount: delinquentInvoices.length } };
  ...
  const handleDeepDive = async (chartId: string, chartTitle: string, chartData: any) => {
    ...
    if (chartId === 'cashflow') {
      ...
      dataForAi = chartData;
    } else if (chartId === 'retention') {
      const now = new Date();
      const healthLedger = filteredInvoices
        .filter(i => i.type === 'Invoice' && !['Pagada', 'Rechazada', 'Borrador', 'Incobrable'].includes(i.status))
        .map(i => {
          const due = i.dueDate ? new Date(i.dueDate) : new Date(new Date(i.date).getTime() + 15 * 24 * 60 * 60 * 1000);
          const isDelinquent = due < now;
          return {
            id: i.id,
            client: i.clientName,
            total: i.total,
            dueDate: i.dueDate || due.toISOString().split('T')[0],
            status: isDelinquent ? 'Morosa' : 'Al Día',
            daysLate: isDelinquent ? Math.floor((now.getTime() - due.getTime()) / (1000 * 3600 * 24)) : 0
          };
        })
  ...
  const fetchDeepDiveReport = async (chartTitle: string, chartData: any, type: string) => {
    ...
    try {
      if (type === 'cashflow') {
        ...
        context = `
              ANÁLISIS DE SALUD DE CARTERA - ${timeRange}
              ------------------------------------------
              FACTURAS AL DÍA: ${data.kpis.healthyCount}
              FACTURAS MOROSAS: ${data.kpis.delinquentCount}
              
              INSTRUCCIÓN ESPECIAL PARA IA:
              Actúa como Analista de Cobranzas. Evalúa el riesgo de flujo de caja.
              Analiza la relación entre facturas al día vs morosas.
              
              GENERAR JSON con estos campos obligatorios:
              1. 'chartTitle': "${chartTitle}"
              2. 'executiveSummary': Resumen del estado de la cartera.
                ...
                <tbody className="divide-y divide-slate-50">
                  {deepDiveVisual.data.map((row: any, i: number) => (
                    <tr key={i} className="hover:bg-slate-50/50 group">
                      ...
                      <td className="p-3 text-right text-slate-500">{new Date(row.dueDate).toLocaleDateString()}</td>
                      <td className="p-3 text-center">
                        <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${row.status === 'Morosa' ? 'bg-red-50 text-red-600' : 'bg-emerald-50 text-emerald-600'
                          }`}>
                          {row.status}
                        </span>
                      </td>
                      <td className="p-3 text-right text-rose-500 font-medium">{row.daysLate > 0 ? `${row.daysLate}d` : '-'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
    ...
    <div ref={clientsRef} className="p-4 bg-slate-50/50 rounded-[3rem] -m-4">
      ...
      <div className="p-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
          <div className="bg-white p-5 rounded-3xl shadow-sm border border-slate-50 flex items-center justify-between">
            <div>
              <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Facturas al Día</p>
              <h3 className="text-2xl font-bold text-[#27bea5]">{data.kpis.healthyCount}</h3>
              <span className="text-[10px] text-slate-400">Dentro del plazo de cobro</span>
            </div>
            <div className="p-3 bg-teal-50 rounded-full text-[#27bea5]">
              <Check className="w-6 h-6" />
            </div>
          </div>
          <div className="bg-white p-5 rounded-3xl shadow-sm border border-slate-50 flex items-center justify-between">
            <div>
              <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Facturas Morosas</p>
              <h3 className="text-2xl font-bold text-rose-500">{data.kpis.delinquentCount}</h3>
              <span className="text-[10px] text-slate-400">Vencimiento superado</span>
            </div>
          ...
          <div className="bg-white p-6 md:p-8 rounded-[2.5rem] shadow-sm border border-slate-50 hover:shadow-md transition-shadow">
            <div className="flex justify-between items-start mb-6">
              <div>
                <h3 className="font-bold text-[#1c2938] text-xl flex items-center gap-2">
                  ...
                  Salud de Cartera (Vencimientos)
                </h3>
                <p className="text-slate-400 text-sm mt-1 ml-11">Relación entre facturas vigentes vs morosas por fecha de vencimiento.</p>
              </div>
              <button id="no-print" onClick={() => handleDeepDive('retention', 'Salud de Cartera', data.portfolioHealthData)} className="p-3 rounded-xl bg-slate-50 hover:text-emerald-500 transition-all">
            ...
            <div className="h-72 w-full flex items-center justify-center">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie
                    ...
                    dataKey="value"
                  >
                    {data.portfolioHealthData.map((entry: any, index: number) => (
              ...
              <div className="absolute flex flex-col items-center justify-center">
                <span className="text-3xl font-bold text-[#1c2938]">{data.kpis.healthyCount + data.kpis.delinquentCount}</span>
                <span className="text-[10px] text-slate-400 uppercase font-bold tracking-widest">Docs. Pendientes</span>
              </div>
            </div>
          </div>
        </div>
              ...
              <div className="space-y-4">
                {fiscalData.insights.length > 0 ? (
                  fiscalData.insights.map((insight, idx) => (
                    <div key={idx} className={`p-4 rounded-2xl border flex gap-3 ${insight.type === 'alert' ? 'bg-red-50 border-red-100 text-red-800' :
        ...
        <div className="bg-white p-1.5 rounded-2xl shadow-sm border border-slate-100 flex overflow-x-auto max-w-full custom-scrollbar">
          {(['OVERVIEW', 'DOCUMENTS', 'CLIENTS', 'FISCAL'] as const).map(tab => (
            <button
              ...
              className={`px-6 py-3 rounded-xl text-sm font-bold transition-all duration-300 whitespace-nowrap flex items-center gap-2 ${activeTab === tab
                ? 'bg-[#1c2938] text-white shadow-md'
