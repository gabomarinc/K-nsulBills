        ...
        return (
            <div className="max-w-2xl mx-auto p-6 bg-white rounded-3xl shadow-lg text-center mt-10">
                ...
                <div className="border-2 border-dashed border-slate-200 rounded-2xl p-10 flex flex-col items-center justify-center gap-4 hover:bg-slate-50 transition-colors relative min-h-[300px] overflow-hidden">
                    ...
                    {isLoading ? (
                        <div className="flex flex-col items-center justify-center z-20 relative w-full h-full min-h-[300px]">
                            {uploadedImage && (
                                fileType === 'pdf' ? (
                                    <div className="absolute inset-0 flex flex-col items-center justify-center bg-slate-50/90 rounded-2xl">
                                        <FileText className="w-16 h-16 text-slate-400 mb-2" />
                                        <p className="text-sm font-bold text-slate-500">Documento PDF</p>
                                    </div>
                                ) : (
                                    <img src={uploadedImage} alt="Preview" className="absolute inset-0 w-full h-full object-contain rounded-2xl opacity-60" />
                                )
                            )}
                            <div className="relative z-30 bg-white/90 p-6 rounded-2xl shadow-xl border border-slate-100 flex flex-col items-center backdrop-blur-sm">
                                <Loader2 className="w-10 h-10 animate-spin text-[#27bea5] mb-2" />
                                <p className="text-[#1c2938] font-bold animate-pulse">{loadingMsg}</p>
                            </div>
                        </div>
                    ) : (
                        <>
