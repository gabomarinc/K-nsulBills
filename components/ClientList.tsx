...
const ClientList: React.FC<ClientListProps> = ({ invoices, dbClients = [], onSelectClient, onCreateDocument, onCreateClient, currencySymbol, currentUser }) => {
  ...
  const { clients, stats } = useMemo(() => {
    ...
    dbClients.forEach(dbClient => {
      if (!dbClient.name) return;
      const nameKey = dbClient.name.trim().toLowerCase();
      clientMap.set(nameKey, {
        name: dbClient.name.trim(),
    ...
    invoices.forEach(inv => {
      ...
      const nameKey = inv.clientName.trim().toLowerCase();
      ...
      if (!clientMap.has(nameKey)) {
        clientMap.set(nameKey, {
          ...
          isVip: false,
          winRate: 0,
          fullData: { name: inv.clientName, taxId: inv.clientTaxId }
        });
      }
      ...
      if (inv.clientTaxId && (!client.taxId || client.taxId === 'N/A')) {
        client.taxId = inv.clientTaxId;
      }
